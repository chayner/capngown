...
- **Staff notes on graduates**: New `graduates.notes` text column (migration `add_notes_to_graduates`) lets staff record exceptions like "needs second hood" or "see Chip for custom order". Editable from the new graduate edit form. The graduate show page renders a prominent amber callout when a note exists; the printed sticker shows a discreet `support_agent` icon + "See Staff" indicator (intentionally neutral wording so it isn't awkward for the recipient, but distinct from existing brag/cord indicators so volunteers spot it). New `Graduate#notes?` predicate. Three controller tests added.
