...
- **`graduates.levelcode` is `"UG"` or `"GR"` only** — production rosters do NOT distinguish master's vs doctorate via `levelcode` (no `GR-M` / `GR-D` values exist). Always derive level from `degree1` using `Graduate.master` / `Graduate.doctorate` / `Graduate.undergraduate` scopes (backed by `DegreeHoodTranslator::DEGREE_HOOD_MAP[:level]`). Filtering on `levelcode: "GR-M"` returns zero rows silently — that bug hid the entire master/doctorate breakdown on the stats dashboard.
